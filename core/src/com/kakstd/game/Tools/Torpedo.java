...
public abstract class Torpedo extends Sprite {
    ...
    public Torpedo (World world, PlayScreen screen, Enemy npc, float knobX, float knobY){
        super(screen.getAtlas().findRegion("Torpedo_Meksikano"));
        setOriginCenter();
        torpedoTex = new TextureRegion(getTexture(),700,16, 16,16);
        this.world = world;
        this.npc = npc;
        speed = 4;
    ...
    public Torpedo (World world, PlayScreen screen, Player player, float knobX, float knobY){
        super(screen.getAtlas().findRegion("Torpedo_Meksikano"));
        setOriginCenter();
        torpedoTex = new TextureRegion(getTexture(),700,16, 16,16);
        this.world = world;
    ...
    public void draw(Batch batch){
        batch.draw(torpedoTex, b2d.getPosition().x-(getWidth()/2)/SubmarineGame.PPM, b2d.getPosition().y-(getHeight()/2)/SubmarineGame.PPM, (getWidth()/2)/SubmarineGame.PPM,
                ...
                getHeight()/SubmarineGame.PPM, 1,1,rt);
    }
